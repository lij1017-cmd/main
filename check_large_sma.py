...
class Backtester:
    ...
    def run(self, sma_period, roc_period, stop_loss_pct, rebalance_interval, start_date, end_date):
        ...
        # MKT Filter: Weighted Average of ROC? No, keep it simple.
        mkt_ma = self.prices_df.mean(axis=1).rolling(window=200).mean()
        mkt_price = self.prices_df.mean(axis=1)
        mkt_up = mkt_price > mkt_ma
    ...
    for sl in [0.08]:
        for sma in [150, 200]:
            for roc in [150, 200]:
                ...
                print(f"SMA:{sma} ROC:{roc} -> MDD:{m:.2%} Calmar:{cal:.2f}")
