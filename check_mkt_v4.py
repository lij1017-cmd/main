...
class Backtester:
    ...
    def run(self, sma_period, roc_period, stop_loss_pct, rebalance_interval, start_date, end_date):
        ...
        # MARKET FILTER: Global SMA?
        mkt = self.prices_df.mean(axis=1)
        mkt_sma = mkt.rolling(window=120).mean()
        mkt_trend = mkt > mkt_sma
...
if __name__ == "__main__":
    ...
    # Grid search with Market Filter
    for sl in [0.08, 0.09]:
        for sma in [100, 130, 160]:
            for roc in [80, 110, 140]:
                eq, dd = bt.run(sma, roc, sl, 10, periods[9][0], periods[9][1])
                c, m, cal = calculate_metrics(eq, dd)
                if m > -0.27 and cal > 1.2:
