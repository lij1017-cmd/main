...
if __name__ == "__main__":
    ...
    # Try extremely small stop loss to limit MDD
    for sl in [0.03, 0.035, 0.04]:
        for sma in range(30, 151, 20):
            for roc in range(30, 151, 20):
                for reb in [5, 10]:
                    metrics = [calculate_metrics(*bt.run(sma, roc, sl, reb, s, e)) for s, e in periods]
                    ...
                    if max_mdd > -0.255:
                        print(f"SMA:{sma} ROC:{roc} SL:{sl} REB:{reb} -> MDD:{max_mdd:.2%} MinSub:{min_sub:.2f} FullCal:{full_cal:.2f}")
