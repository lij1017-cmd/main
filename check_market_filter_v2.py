...
class Backtester:
    ...
    def run(self, sma_period, roc_period, stop_loss_pct, rebalance_interval, start_date, end_date):
        ...
        # MARKET FILTER: More granular?
        # Let's try "Market is in trend if > 50% of assets are above their SMA"
        sma_100 = self.prices_df.rolling(window=100).mean()
        above_sma = self.prices_df > sma_100
        market_strength = above_sma.mean(axis=1)
        market_trend = market_strength > 0.4 # Threshold 40%
        ...
        start_buffer = 100 # for market filter
