...
class Backtester:
    ...
    def run(self, sma_period, roc_period, stop_loss_pct, rebalance_interval, start_date, end_date):
        ...
        # Taiwan Index proxy (Mean of all assets)
        tw_index = self.prices_df.mean(axis=1)
        tw_sma = tw_index.rolling(window=100).mean()
        tw_trend = tw_index > tw_sma
        ...
        start_buffer = max(int(sma_period), int(roc_period), 100)
        ...
        for i in range(loop_start, last_idx + 1):
            ...
            if (i - loop_start) % int(rebalance_interval) == 0:
                ...
                # Rule: Rebalance only if Market Trend is UP
                if not tw_trend.iloc[i]:
                    # Go to cash
                    for s_id, info in slots.items():
                        if info:
                            surplus_pool += info['shares'] * next_prices[info['asset_idx']] * (1 - 0.001425 - 0.003)
                    ...
                    for s_id in range(3):
                        if s_id not in kept_sids.values():
                            if slots[s_id]:
                                proceeds = slots[s_id]['shares'] * next_prices[slots[s_id]['asset_idx']] * (1 - 0.001425 - 0.003)
                                budgets.append(min(proceeds, 10000000.0))
                                slots[s_id] = None
...
if __name__ == "__main__":
    ...
    print("Testing Large SMA/ROC with Market Filter...")
    for sl in [0.08, 0.09]:
        for reb in [10, 12]:
            for sma in [100, 150, 200]:
                for roc in [100, 150, 200]:
                    eq, dd = bt.run(sma, roc, sl, reb, periods[9][0], periods[9][1])
