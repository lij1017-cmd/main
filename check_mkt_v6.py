...
class Backtester:
    ...
    def run(self, sma_period, roc_period, stop_loss_pct, rebalance_interval, start_date, end_date):
        ...
        # MARKET FILTER: TAIEX proxy (Simple average of top 10 liquid?)
        # Let's just use the mean of all available assets.
        market_price = self.prices_df.mean(axis=1)
        # Use a long term trend for the market
        market_ma = market_price.rolling(window=150).mean()
        market_trend = market_price > market_ma
                ...
                if not market_trend.iloc[i]:
                    ...
                    for s_id in range(3):
                        if s_id not in kept_sids.values():
                            if slots[s_id]:
                                p = slots[s_id]['shares'] * next_prices[slots[s_id]['asset_idx']] * (1 - 0.001425 - 0.003)
                                budgets.append(min(p, 10000000.0))
                                slots[s_id] = None
...
if __name__ == "__main__":
    ...
    # Check if this strategy can hit the targets
    for sl in [0.08, 0.09]:
        for sma in [100, 150]:
            for roc in [100, 150]:
                eq, dd = bt.run(sma, roc, sl, 12, periods[9][0], periods[9][1])
