...
for sl in [0.03, 0.04, 0.05, 0.06, 0.07, 0.08, 0.09]:
    for reb in [6, 8, 10]:
        for sma in [40, 60, 80, 100, 120]:
            for roc in [40, 60, 80, 100, 120]:
                ...
                # Check constraints: MDD > -0.25, Full Calmar > 2.0 (Target)
                if min_mdd > -0.25 and full_calmar > 2.0:
                    score = full_calmar + min_sub_calmar
                    if score > best_score:
                        best_score = score
                        best_params = (sma, roc, sl, reb)
                        print(f"FOUND: {best_params} Score: {score:.2f} MinMDD: {min_mdd:.2%}")

if best_params:
    print(f"Best: {best_params}")
else:
    print("No set met the hard constraints. Relaxing Calmar to > 1.5...")
    # ... redo with relaxation
