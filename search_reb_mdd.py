...
if __name__ == "__main__":
    ...
    # Try different rebalance and stop loss combinations specifically for 2022-2024
    for reb in [15, 20, 30]:
        for sl in [0.03, 0.05, 0.07, 0.09]:
            for sma in [100, 150, 200]:
                eq, dd = bt.run(sma, 100, sl, reb, periods[9][0], periods[9][1])
                c, m, cal = calculate_metrics(eq, dd)
                if m > -0.27 and cal > 1.2:
                    print(f"SMA:{sma} SL:{sl} REB:{reb} -> MDD:{m:.2%} Calmar:{cal:.2f}")
