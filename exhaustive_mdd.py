...
if __name__ == "__main__":
    ...
    print("Final MDD Targeted Grid Search...", flush=True)
    # Step through Stop Loss and SMA/ROC
    for sl in [0.03, 0.04, 0.05, 0.06, 0.07, 0.08]:
        for reb in [10, 11, 12]:
            for sma in range(100, 181, 20):
                for roc in range(100, 181, 20):
                    eq_f, dd_f = bt.run(sma, roc, sl, reb, periods[9][0], periods[9][1])
                    c_f, m_f, cal_f = calculate_metrics(eq_f, dd_f)
                    if m_f > -0.25 and cal_f > 1.3:
                        metrics = [calculate_metrics(*bt.run(sma, roc, sl, reb, s, e)) for s, e in periods[:9]]
                        mdds = [m[1] for m in metrics]
                        calmars = [m[2] for m in metrics]
                        max_mdd = min(mdds + [m_f])
                        min_cal = min(calmars)
                        if max_mdd > -0.25 and cal_f > 1.5:
                            print(f"SMA:{sma} ROC:{roc} SL:{sl} REB:{reb} -> MaxDD:{max_mdd:.2%} MinCal:{min_cal:.2f} FullCal:{cal_f:.2f}", flush=True)
                            if cal_f > 2.0 and min_cal >= 1.5:
