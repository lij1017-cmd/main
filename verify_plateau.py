...
def main():
    ...
    for roc, ap, am in itertools.product(rocs, aps, ams):
        ...
        res = run_sim(bt, p)
        if res:
            row = {'roc': roc, 'atr_p': ap, 'atr_m': am,
                   'Full_CAGR': res['Full_CAGR'], 'Full_Calmar': res['Full_Calmar'], 'Full_MaxDD': res['Full_MaxDD']}
            for y, r in res['years'].items():
                row[f'Ret_{y}'] = r
            results.append(row)

    df = pd.DataFrame(results)
    df.to_csv('plateau_check.csv', index=False)
    print("Plateau check saved to plateau_check.csv")
