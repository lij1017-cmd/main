...
if __name__ == "__main__":
    ...
    for sl in [0.03, 0.05, 0.07, 0.09]:
        for reb in [5, 7, 10, 12]:
            for sma in [40, 70, 100, 130]:
                for roc in [40, 70, 100, 130]:
                    eq_f, dd_f = bt.run(sma, roc, sl, reb, periods[9][0], periods[9][1])
                    c_f, m_f, cal_f = calculate_metrics(eq_f, dd_f)
                    if m_f > -0.26 and cal_f > 1.8:
                        # Check sub-periods
                        mdds = []
                        calmars = []
                        for s, e in periods[:9]:
                        ...
                        min_cal = min(calmars)
                        if max_mdd > -0.26:
                            print(f"WINNER: SMA:{sma} ROC:{roc} SL:{sl} REB:{reb} -> MDD:{max_mdd:.2%} MinCal:{min_cal:.2f} FullCal:{cal_f:.2f}")
