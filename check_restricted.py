    ...
    def run(self, sma_period, roc_period, stop_loss_pct, rebalance_interval, start_date, end_date):
        ...
        for i in range(loop_start, last_idx + 1):
            ...
            if (i - loop_start) % int(rebalance_interval) == 0:
                ...
                # REBALANCE LOGIC: Check how many meet filter
                candidates = []
                sorted_all = np.argsort(roc[i])[::-1]
                for idx in sorted_all:
                    if len(candidates) >= 3: break
                    if current_prices[idx] > sma[i][idx] and roc[i][idx] > 0: candidates.append(idx)

                # Rule: If only 1 meets criteria, allocate only 10M, leave 20M in cash.
                # If 0 meets, 30M in cash.
                # This naturally limits MDD during bad markets.

                kept_sids = {info['asset_idx']: s_id for s_id, info in slots.items() if info and info['asset_idx'] in candidates}
                kept_mv = sum(slots[sid]['shares'] * current_prices[sig] for sig, sid in kept_sids.items())

                # The total pool for THIS rebalance cycle is 30M.
                # We only spend on assets in `candidates`.
                ...
                for s_id in range(3):
                    if s_id not in kept_sids.values():
                        if slots[s_id]:
                            slots[s_id] = None # Position sold at T+1 (proceeds already added to pool above in SL or just handled by reset)
                        budgets.append(10000000.0)

                new_signals = [sig for sig in candidates if sig not in kept_sids]
...
if __name__ == "__main__":
    ...
    # Grid search with restricted allocation
    for sl in [0.08, 0.09]:
        for sma in [100, 120, 140]:
            for roc in [80, 100, 120]:
                eq, dd = bt.run(sma, roc, sl, 10, periods[9][0], periods[9][1])
                c, m, cal = calculate_metrics(eq, dd)
                print(f"SMA:{sma} ROC:{roc} SL:{sl} -> MDD:{m:.2%} Calmar:{cal:.2f}")
