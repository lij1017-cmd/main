...
if __name__ == "__main__":
    ...
    # Eval candidate (90, 90, 0.05, 12) - slightly extended from 20 to 12
    for reb in [10, 11, 12]:
        for sl in [0.045, 0.05, 0.055]:
            for sma in [80, 90, 100]:
                for roc in [80, 90, 100]:
                    calmars = []
                    mdds = []
                    for s, e in periods:
                        eq, dd = bt.run(sma, roc, sl, reb, s.replace('.','-'), e.replace('.','-'))
                        c, m, cal = calculate_metrics(eq, dd)
                        calmars.append(cal)
                        mdds.append(m)
                    min_sub = min(calmars[:9])
                    full_c = calmars[9]
                    worst_mdd = min(mdds)
                    if full_c > 1.8 and worst_mdd > -0.27:
                        print(f"SMA:{sma} ROC:{roc} SL:{sl} REB:{reb} -> MinSub:{min_sub:.2f} Full:{full_c:.2f} MDD:{worst_mdd:.2%}")
