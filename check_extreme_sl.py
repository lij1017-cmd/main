...
if __name__ == "__main__":
    ...
    print("Testing SL 1.5% - 2.5% Grid...")
    for sl in [0.015, 0.02, 0.025]:
        for reb in [10, 11, 12, 13, 14, 15]:
            for sma in range(30, 201, 20):
                for roc in range(30, 201, 20):
                    eq_f, dd_f = bt.run(sma, roc, sl, reb, periods[9][0], periods[9][1])
                    c_f, m_f, cal_f = calculate_metrics(eq_f, dd_f)
                    if m_f > -0.25 and cal_f > 1.2:
                        print(f"SMA:{sma} ROC:{roc} SL:{sl} REB:{reb} -> MDD:{m_f:.2%} FullCal:{cal_f:.2f}")
