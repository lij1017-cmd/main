...
class BacktesterFilter:
    ...
    def run(self, sma_period, roc_period, stop_loss_pct, rebalance_interval=6, market_sma_period=100, stop_loss_type='peak', ma_stop_period=5):
        # 1. 指標預計算
        ...
        market_sma = market_avg.rolling(window=market_sma_period).mean()
        market_ok = (market_avg > market_sma).values
        ...
        start_idx = max(sma_period, roc_period, 20, market_sma_period)
        ...
        for i in range(start_idx, len(self.dates)):
            ...
            # 市場濾網觸發：如果今日收盤 Market Average <= SMA
            market_exit_triggered = not market_ok[i]
            ...
            for s_id, info in slots.items():
                if info and 'asset_idx' in info:
                    ...
                    if market_exit_triggered:
                        stop_triggered = True
                        reason_str = f"市場濾網觸發：大盤均價低於 SMA({market_sma_period})，進行全清倉"
                    elif stop_loss_type == 'peak':
            ...
            for s_id, reason_str in triggered_slots:
                ...
                current_reasons.append(f"{name}因達賣出標準需剔除({reason_str[:10]}...)")
                ...
                trades_log.append({
                    ...
                    '標的名稱': name, '原因': '市場/停損' if market_exit_triggered else '停損',
                    '買入手續費': 0, '賣出手續費': sell_fee, '賣出交易稅': sell_tax,
                    '說明': f"賣出：{name} ({reason_str})"
                })
                slots[s_id] = None
            ...
            if is_rebalance_day:
                ...
                # 如果市場濾網處於不佳狀態，不買入任何股票
                if not market_ok[i]:
                    top_3_signals = []
                    current_reasons.append("大盤趨勢不佳，市場濾網排除所有買進訊號")
                else:
                    # 篩選訊號
                    top_3_signals = []
                    exclusion_reasons = []
                ...
                if market_ok[i] and len(top_3_signals) < 3:
