...
class Backtester:
    ...
    def run(self, sma_period, roc_period, stop_loss_pct, rebalance_interval, start_date, end_date):
        ...
        # Rule: Only long if > 40% of all assets are above their own SMA 100
        sma_m = self.prices_df.rolling(window=100).mean().values
        above_m = self.prices > sma_m
        strength = above_m.mean(axis=1)
        mkt_up = strength > 0.4
