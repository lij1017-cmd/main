...
if __name__ == "__main__":
    ...
    print("Scanning for parameters with MDD < 25%...")
    for sl in [0.02, 0.03, 0.04]:
        for reb in [10, 20]:
            for sma in [60, 100, 140]:
                for roc in [60, 100, 140]:
                    eq, dd = bt.run(sma, roc, sl, reb, '2019-01-02', '2025-12-31')
                    c, m, cal = calculate_metrics(eq, dd)
                    if m > -0.26:
                        print(f"SMA:{sma} ROC:{roc} SL:{sl} REB:{reb} -> MDD:{m:.2%} Calmar:{cal:.2f} CAGR:{c:.2%}")
